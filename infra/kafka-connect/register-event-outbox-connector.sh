#!/usr/bin/env bash
# Registers event-outbox-connector.json against a running kafka-connect
# (infra/docker-compose.yml). See register-auth-outbox-connector.sh for
# why this is a manual, not automatic, step.
set -euo pipefail

CONNECT_URL="${CONNECT_URL:-http://localhost:8083}"
CONFIG_FILE="$(dirname "$0")/event-outbox-connector.json"
NAME="event-service-outbox-connector"
